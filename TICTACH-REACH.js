...
<div id="task-label" class="task-label hide-label">Baseline 1</div>
...
const timeSpent2 = endTime - startTime2; // Calculate the time spent on the current interval in milliseconds
        ...
        let confirmationMessageText =
          "Continue to silently sense your heartbeat. When the cartoon heart returns, complete the task again.";

        if (baselineCount === 3) {
          confirmationMessageText =
            "<span style='color: darkgreen;'>Baseline Complete. The research assistant will now explain the next step</span>";
  ...
  animationPaused = false;
    ...
    function endTask() {
      if (recordingStarted) {
        ...
        const fileName = `TICTOC_REACH_${subjectId}.txt`; // Customize the file name with subjectId
